...
void display_game(void) {
	...
	case INPUT_START:
		...
		end_move_list = inputMoveStart(chess_board, start_coordinate);
	...
	case INPUT_END:
		...
		{
			chess_board = inputEndMove(chess_board, start_coordinate, end_coordinate);
	...
	if (refresh_display && input_mode != INPUT_PROMOTION) {
		// Display chess board
	...
	else if (refresh_display && input_mode != INPUT_PROMOTION) {
		// Display promotion choices
		// (See promotePawn() in 'ChessMoves.h' for input options)
	}
}
